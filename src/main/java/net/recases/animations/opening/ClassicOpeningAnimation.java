...
public class ClassicOpeningAnimation extends AbstractChestOpeningAnimation {
    ...
    protected void beforeSpawn(OpeningSession session) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                runtime.getLocation().getWorld().spawnParticle(isPremiumReward(session) ? Particle.GLOW : Particle.ENCHANT, runtime.getLocation().clone().add(0.5, 1.0, 0.5), 18, 0.9, 0.4, 0.9, 0.05);
                runtime.getLocation().getWorld().spawnParticle(isPremiumReward(session) ? Particle.END_ROD : Particle.CRIT, runtime.getLocation().clone().add(0.5, 0.6, 0.5), 8, 0.15, 0.15, 0.15, 0.01);
                runtime.getLocation().getWorld().playSound(runtime.getLocation(), isPremiumReward(session) ? Sound.BLOCK_BEACON_POWER_SELECT : Sound.BLOCK_AMETHYST_BLOCK_RESONATE, 0.6F, 0.9F + (pulse * 0.06F));
            }
    ...
    protected void afterSpawn(OpeningSession session) {
        ...
        runtime.getLocation().getWorld().playSound(runtime.getLocation(), isPremiumReward(session) ? Sound.BLOCK_ENDER_CHEST_OPEN : Sound.BLOCK_BEACON_ACTIVATE, 0.8F, 1.1F);
    }
